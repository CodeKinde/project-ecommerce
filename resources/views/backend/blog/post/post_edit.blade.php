...
<form action="{{ route('post.update') }}" method="POST" enctype="multipart/form-data">
            ...
            <input type="hidden" name="old_image" value="{{ $posts->post_image }}">
...
<div class="row">
    ...
    <h5>Title Nom (fr) <span class="text-danger">*</span></h5>
            <div class="controls">
<input  type="text" name="title_fr"  class="form-control" value="{{ $posts->title_fr }}"></div>
        @error('title_fr')
    ...
    <div class="col-md-6">
        ...
        <h5>Title Name (en) <span class="text-danger">*</span></h5>
                <div class="controls">
    <input  type="text" name="title_en"  class="form-control" value="{{ $posts->title_en }}"></div>
        @error('title_en')
        <div class="text-danger">{{ $message }}</div>
        @enderror
      </div>
    ...
    <div class="col-md-6">
      ...
      <select name="blog_category_id" id="blog_category_id" class="form-control">
        ...
        @foreach ($category as $categ)
            <option value="{{ $categ->id }}"{{ $categ->id == $posts->blog_category_id ? 'selected' : ''}}>{{ $categ->blog_category_name_fr }}</option>
        @endforeach
        </select>
            </div>
     ...
     <div class="col-md-6">
        <div class="form-group">
           ...
           <textarea id="editor" name="post_details_fr" rows="5" cols="80">
                    {{ $posts->post_details_fr }}</textarea></div>
        ...
        <div class="col-md-6">
         ...
         <textarea id="editor" name="post_details_en" rows="5" cols="80">
            {{ $posts->post_details_en }}</textarea></div>
